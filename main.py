...
之所以需要這層轉接，是因為 `src/` 不是一個可安裝的套件，得靠
`scripts/bootstrap.py` 把它塞進 sys.path。直接跑 `python src/core/pipeline.py`
是不行的——那樣 repo root 不在 sys.path 上，pipeline 的相依一個都找不到。
把進入點統一在 repo root，使用者就不必知道這件事。

...
**佔位符代入後會自動帶上單位，你不可以再重複寫一次。**
這是最常犯的錯誤，會在簡報上產生「第一名名」這種重複字：
- 單位 `名` 會代入成「第 3 名」。寫「排名第 {{…}} 名」→「排名第第 3 名名」。
  正確寫法：「排名 {{…}}」
- 單位 `%` 會代入成「12.3%」。寫「{{…}}%」→「12.3%%」。
  正確寫法：「{{…}}」
- 其他單位（張、元、件）會附在數字後。寫「{{…}}張」→「4,512,345張張」。
  正確寫法：「{{…}}」
規則：**佔位符前後都不要再寫單位或「第」**。每個指標的單位已列在下方
指標資訊中，據此判斷代入後的樣子。

**一律使用繁體中文（zh-TW），不可夾雜簡體字。**

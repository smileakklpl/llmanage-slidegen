"""SheetMap + xlsx → 表格資料。管線 [1.5] 到 [2] 之間缺的那一棒。

在此之前 SheetMap 沒有任何消費者——模型吐出來就丟掉了，所以沒人知道
那份契約的欄位夠不夠用。這支程式是第一個真的拿它去讀資料的東西。

**一支 reader 吃下所有工作表**，靠的是 SheetMap 描述的形狀參數。
附件四的四張表只差三個參數（合計列位置、有無衍生欄、列排序），
所以不需要寫四支 py——這正是 SheetMap 存在的理由。

合計列的處理是這裡最關鍵的一行：`rows` 一律排除 `total_row`。
metric_definitions.json 記載，合計列若混進一般機構會佔據第一名
並使其後所有名次位移，那正是附件三 P.7 錯誤 #6/#7 的成因。
"""

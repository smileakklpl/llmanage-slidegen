...
1. **必須同時給頭尾各數列。** 附件四 P.5 的「總計」在第 34 列、P.7 在第 2 列。
   只給前 6 列，模型會漏掉 P.5 的合計列，把它當成一般機構納入排名。

2. **不能相信 openpyxl 的 max_column。** P.5_流通卡數 回報 15 欄，
   但實際有值的只到第 13 欄——尾端兩欄有格式沒內容。以實際有值的最右欄為準。

3. **樣本值只放前後各數列且截斷長度。** 樣本的用途是讓模型判斷欄位型別與位置，
   不是給它推論用的資料。system prompt 要明確禁止用樣本做任何敘述。
"""
